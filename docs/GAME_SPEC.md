...
Inventoryの「！」は既存条件で発生した未確認内容を示す。Inventory入口・各Tab・個別通知は独立した段階として扱う。Inventory／対象Tabを表示中は通知を維持し、閉じる／別TabやMergeへ離れる時点で、その段階を既読にする。入口を既読にしても未訪問Tabや未表示Cardは既読にしない。

Dumbbell／Auraの既存Buy通知はItemsと同じ「実Viewport表示 → 対象一覧から離脱時Seen」とする。表示直後には消さず、実際に表示したIDだけを記録する。画面外のCardはTab離脱・Close後も未読。Dumbbellの購入可否はDumbbellServiceの正規結果、Auraは既存の未所有・Win購入可能条件を維持する。Speedは既存Tab通知のみで、個別Card通知やPremium／Entitlement通知を追加しない。購入・Equip等で既存通知条件が解消した場合はその結果を優先する。

保存は既存InventoryNotificationSeenの各集合を再利用する。Dumbbellsの通常IDはTab、DumbbellBuyのIDはBuy、Auraの通常IDはBuy・Tab:IDはTab、Speedの通常IDはTab。各カテゴリ内のInventory:IDで入口を分離し、旧形式の既読IDを引き継ぐ。Itemsの保存形式は維持。新しいDataStore／Schemaは追加しない。
