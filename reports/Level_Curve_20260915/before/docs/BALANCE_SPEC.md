...
2026-09-15：下表の確定値をReplicatedStorage.Config.World2Config.Stagesへ反映。World2BossConfig / World2WallConfigはその参照用Config。World2HPScaleは参照2箇所の置換後に削除済み。World1値・Level式は変更していない。現行Level式ではLv200閾値622,000、Strength5BはLv22,361となり、推奨Lvと育成カーブの対応は未解決。設定値の検証完了はLv200クリア・World2 Combat完成を意味しない。[検証報告](../reports/World2_Balance_20260915/build_report.md)。
