...
Strengthはサーバー属性を受けてNumberFormatで短縮表示。Levelは現在Strengthの閾値から計算する。バー比率は `(Strength - 現Level開始閾値) / (次Level閾値 - 現Level開始閾値)`。数値ラベルの現在Strength/次閾値とバーの区間内進捗は異なる。

RebirthでStrengthが0になるとLevel1・対応バーへ更新。Potionは有効種別と残り時間を表示。表示上の丸めをゲーム内部のStrengthへ書き戻さない。
