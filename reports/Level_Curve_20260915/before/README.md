...
World1 Stage1〜10の進行、後半Balance、大型Boss Collider、HP UIを実装済み。World2はGate表示・解放データ・Map Grayboxまでで、移動・Combat・Stage進行は未実装。World2専用Balance（推奨Lv60〜200 / Boss15M〜25B）はConfig反映・Play数値検証済み。現行Level式との不一致は要確認。World2専用Inventoryは設計確定・未実装。英語名と公開タイトルの統一は未確認。
...
現在の未確定事項はBossへのCarryと初期Stageの推奨Lv差。World2永続解放はStage10 RewardPad取得で確定。次はWorld2のLevelカーブとの不一致、専用Inventory/Item、Enemy Assetを整理し、別途指示された範囲で移動・Combat・Stage進行を進める。今回のConfig更新で次機能への着手は行わない。
