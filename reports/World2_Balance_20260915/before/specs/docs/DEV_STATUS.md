...
- World2はGraybox・表示・保存権限の基盤まで。移動・Combat・Stage進行は未実装。World2専用Balance（Lv60〜200、Boss15M〜25B）とWorld2専用Inventory方針は設計確定・Studio未反映。
...
4. World2再開時は、BALANCE_SPECのWorld2確定設計案をStudio Configへ反映し、World2専用Inventory/Item設計とEnemy Assetを確定したうえで、限定Phaseで移動→Combat→Stage進行を接続する。
...
| K09 | World2 / High・設計確定・未反映 | Studio HPは旧World1基準×25,000のまま。新設計はLv60〜200、RequiredStrength 3M〜5B、Boss 15M〜25B。World2専用Inventoryを到達時解放。World1 Itemは持込・装備可で性能維持 | 新設計をStudio Configへ反映後、World2 Item/Enemy、移動、Combat、Stage進行を実装 |
...
- World2新BalanceのStudio反映、World2専用Item/Inventory実装、Enemy Asset選定、移動、Combat、Stage進行。
